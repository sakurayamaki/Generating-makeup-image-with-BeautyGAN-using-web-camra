...
def build_generator_res(input_A, input_B,name="generator", alpha=0.5):
    with tf.variable_scope(name):
        ks = 3
        fs = 7
        input_pad_A = tf.pad(input_A,[[0,0],[3,3],[3,3],[0,0]],"REFLECT")
        ...
        alpha_c4 = generate_deconv2d(inputdeconv=o_r9_A,o_d=ngf*2,kernal_size=ks,stride=2,padding="SAME",
                                 name="Alpha_c4",stddev=0.02)
        
        alpha_c4 = tf.nn.max_pool(alpha_c4, [1,2,2,1],[1,1,1,1],padding='VALID')
        
        alpha_c5 = generate_deconv2d(inputdeconv=alpha_c4,o_d=ngf,kernal_size=ks,stride=2,padding="SAME",
                                 name="Alpha_c5",stddev=0.02)
        
        alpha_c5 = tf.nn.max_pool(alpha_c5, [1,2,2,1],[1,1,1,1],padding='VALID')
        
        alpha_c6 = generate_deconv2d(inputdeconv=alpha_c5,o_d=1,kernal_size=fs,stride=1,padding="SAME",
                                 name="Alpha_c6",stddev=0.02,do_relu=False)
        
        alpha = tf.nn.sigmoid(alpha_c6,name="alpha") 
        
        alpha = tf.image.resize_images(alpha, [256,256]) 
        ...
        makeup_mask = tf.nn.tanh(A_c6,name="A_t")
        
        out_gen_A =  makeup_mask * alpha + input_A * (1-alpha)
        
        out_gen_B = tf.nn.tanh(B_c6,name="B_t") 
        
        out_makeup_mask = makeup_mask * alpha + (1-alpha)
        
    return out_gen_A, out_gen_B, out_makeup_mask, alpha
